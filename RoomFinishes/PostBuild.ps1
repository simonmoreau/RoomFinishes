...
function CopyToFolder($revitVersion, $addinFolder) {
    ...
    if (Test-Path $addinFolder) {
        try {
            ...
            if (Test-Path ($addinFolder  + "\RoomFinishes.addin")) { Remove-Item ($addinFolder  + "\RoomFinishes.addin") }
            ...
            # Copy the addin file
            xcopy /Y ($ProjectDir + "RoomFinishes.addin") ($addinFolder)
...
$ReleasePath="G:\My Drive\05 - Travail\Revit Dev\RoomFinishes\Release"
